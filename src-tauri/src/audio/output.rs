...
use super::playback::{OutputSignal, PlaybackId};
...
pub(crate) fn build_output_stream(
    ...
    let stream = match sample_format {
        SampleFormat::F32 => build_stream::<f32>(&device, config, playback_id, pcm, signal_sender)?,
        SampleFormat::F64 => build_stream::<f64>(&device, config, playback_id, pcm, signal_sender)?,
        SampleFormat::I8 => build_stream::<i8>(&device, config, playback_id, pcm, signal_sender)?,
        SampleFormat::I16 => build_stream::<i16>(&device, config, playback_id, pcm, signal_sender)?,
        SampleFormat::I24 => {
            build_stream::<cpal::I24>(&device, config, playback_id, pcm, signal_sender)?
        }
        SampleFormat::I32 => build_stream::<i32>(&device, config, playback_id, pcm, signal_sender)?,
        SampleFormat::I64 => build_stream::<i64>(&device, config, playback_id, pcm, signal_sender)?,
        SampleFormat::U8 => build_stream::<u8>(&device, config, playback_id, pcm, signal_sender)?,
        SampleFormat::U16 => build_stream::<u16>(&device, config, playback_id, pcm, signal_sender)?,
        SampleFormat::U24 => {
            build_stream::<cpal::U24>(&device, config, playback_id, pcm, signal_sender)?
        }
        SampleFormat::U32 => build_stream::<u32>(&device, config, playback_id, pcm, signal_sender)?,
        SampleFormat::U64 => build_stream::<u64>(&device, config, playback_id, pcm, signal_sender)?,
        ...
        .build_output_stream(
            ...
            move |output: &mut [T], info| {
                ...
                let Some(end_time) = calculate_end_time(
                    ...
                    let _ = signal_sender
                        .try_send(OutputSignal::CompletionTimingFailed { playback_id });
                    return;
                };
                let _ = signal_sender.try_send(OutputSignal::FinalFramesSubmitted {
                    playback_id,
                    end_time,
                });
            },
            move |_error| {
                let _ = error_sender.try_send(OutputSignal::StreamFailed { playback_id });
            },
            None,
        )
