...
@Figma その方針でviewごとにFigmaにインポート可能なSVGを作成し、それぞれのSVGをInkscapeのCLIで4080×2700 pxのPNGへレンダリングし、zipにまとめて出力して。
Figmaサーバーにはインタラクトしない。
...
実装者に判断の余地が残らない簡潔な実装計画を作成して。変更・作成されるファイル構造、各ファイルのコンテンツ・コード構造、API契約を固定する。
根拠のない推測は禁止されているため、公式ドキュメント、コードベースを参照して。
