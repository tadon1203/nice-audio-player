...
export { readNativeErrorCode } from "./native-error";
...
export function validateSender<TArgs extends unknown[], T>(
  ...
  return async (
    ...
    const senderIsExpected =
      trustedWindow === undefined ||
      (trustedWindow !== null && trustedWindow.webContents.id === event.sender.id);
    if (!senderIsExpected || !event.senderFrame || !isTrustedRendererUrl(event.senderFrame.url)) {
      return {
        ok: false,
